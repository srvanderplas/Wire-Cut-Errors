...
![Wire cutter with 1.5 cm cutting surface beveled on both sides](../fig/wire_cutter.png){#fig-wire-cutter alt="Wire cutter with 1.5 cm cutting surface beveled on both sides"}
...
Let us first consider the simplest situation where a single tool is recovered from the suspect's house that is viewed as likely to have made the wire cut. 
In order to compare tool marks in this scenario, the examiner or algorithm completes the equivalent to a database search resulting from comparing a single wire in evidence to a single tool. 
This multiple comparison problem becomes more severe at each step of our thought experiment - considering a single shop of tools adds multiple orders of magnitude to the number of comparisons. 
