...
\Cref{fig:Accuracy} shows the family-wise accuracy rate on the y-axis; if 90\% is an acceptable overall accuracy, 2 comparisons can be made when $e = 0.05$; this increases to 10 comparisons when $e =  0.01$.
Thus, at an error rate of \Sexpr{sprintf("e = %.2f", fpr)}, examiners can conduct approximately 5 comparisons without exceeding the family-wise FPR of 10\%.
...
First, examiners should report (or defense attorneys should request) the overall length or area of surfaces generated during the examination process.
% from blade cuts, test fires, and equivalent procedures intended to assess the marks made by a tool submitted for comparison with evidence.
Examiners should also report the total consecutive length or area of the recovered evidence.
...
For instance, we have pooled studies looking at bullet striations and firing pin shear marks.
The striated surfaces are of orders of magnitude different lengths, but they represent our best estimate of the error rate for striated comparisons.
