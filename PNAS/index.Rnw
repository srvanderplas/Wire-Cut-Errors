...
\begin{abstract}
Wires have striations left from the cutting process; as in other forms of toolmark analysis, these striae are used to connect the wire to the tool used. In this paper, we show that the process of wire comparison introduces complexities not present in better-investigated forensic toolmark areas such as bullets, as wire comparison inherently requires multiple distinct comparisons, increasing the expected false positive rate. We call attention to the multiple comparison problem in wire examination and relate it to other situations in forensics that involve multiple comparisons, such as database searches.
\end{abstract}
...
This raises the critically important problem of close non-matches\citep{pcast}.
...
\svp{Compounding this issue,} algorithms make a large number of comparisons that are not obvious to the user.
The cross-correlation function\citep{vorburgerApplicationsCrosscorrelationFunctions2011} was one of the first quantitative measures proposed in response to the NAS report \citep{nas2009}, and is used in many pattern searching algorithms to find the best alignment between two sources and also evaluate the overall similarity.
...
These comparisons are of course highly dependent, as each sequential comparison shares much of the same data with the previous and next comparisons.
...
These comparisons are required in order to find the optimal alignment between the wire and the blade cut; they are implicit in the calculation of cross-correlation which is either the first or the only step used to objectively assess the similarity between striated evidence such as bullets, aperture shear, and firing pin impressions.
...
\svp{\Cref{fig:Accuracy} shows $P(\overline{F_n})$ on the y-axis; if we set 90\% as the acceptable probability, we can make 2 comparisons when the single-comparison FPR is 0.05 and 10 comparisons at 0.01. Thus, at an error rate of \Sexpr{sprintf("%.2f", fpr)}, we can conduct approximately 5 comparisons while keeping the FPR for all comparisons under 10\%.}

\svp{It is clear that under these constraints, the} accuracy of the aggregate comparison between a wire and a tool becomes questionable, as the number of candidate alignments that must be examined is too high.
