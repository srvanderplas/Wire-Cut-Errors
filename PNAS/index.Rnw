...
\ac{Omission of the underlying} details is not necessarily \ac{due to} malice; rather, it results from protocols which focus on finding the best possible match between two items and then evaluating that match, and also from lack of familiarity with basic statistical ideas.
