...
In practice, these algorithms then allow searches across (vast) databases and report the best $N$ matches to a particular piece of evidence, raising the critically important problem of close non-matches\citep{pcast}.
...
Very close non-matches are a direct consequence of a large number of comparisons; they will occur more frequently as databases increase in size and algorithms become more powerful.
...
The cross-correlation function\citep{vorburgerApplicationsCrosscorrelationFunctions2011}, for example, was one of the first quantitative measures proposed in response to the NAS report \citep{nas2009}, and is used in many pattern searching algorithms to find the best alignment between two sources and also evaluate the overall similarity.
Finding the best alignment requires implicitly sliding one source across the whole length (for one-dimensional sources, such as striations) or area (for two dimensional sources, such as impression marks) of the other source while keeping track of the value of a similarity measure.
This mirrors the forensic examination process, as examiners visually rotate or align two pieces of evidence under comparison microscopes.
In order to avoid embarrassing false accusations and a further erosion of the public's trust in science, we need to address the problem of multiple comparisons by controlling their effect on false discovery rates.
...
\svp{An illustration of the sliding comparison process is shown in \Cref{fig:wire-blade}.}
...
When we then consider that examiners would make cuts at multiple angles \citep{baikerToolmarkVariabilityQuality2015}, increasing the number of comparisons even in the simple case we have presented here, the probability of a coincidental match increases further still.
