...
In order to avoid false accusations and the corresponding impact on public perception of forensics, we must address the problem of multiple comparisons and control their effect on false discovery rates.
...
These studies have FPRs between 0.0045 \citep{best2022} and 0.072 \citep{mattijssen2021}; pooling data from these studies weighted by sample size yields an FPR of \Sexpr{sprintf("%.2f", fpr)}.
