...
In order to calculate the number of comparisons carried out in the course of one examination, we define $\ell$ to be the length of the blade cut, and $d$ to be the diameter of the wire. We assume that the wire is covered with striations suitable for comparison across its full diameter $d$. If this is not the case, we reduce the value $d$ correspondingly.
...
At minimum, there must be $\ell/d$ comparisons in order to assess whether one side of the blade cut matches one side of the wire; in this case, the comparisons are physically separate and thus expected to be statistically independent, though empirically there will be nonzero correlations due to the physical similarities between striations.
%no overlapping data is used.
This method of counting the number of comparisons assumes
perfect alignment between the diameter of the wire and the portion of the blade used to cut the wire, which is unrealistic but provides us with a lower bound.
