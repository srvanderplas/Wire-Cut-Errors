...
Statisticians call this the \textit{familywise error rate} ("family" = number of comparisons); it is an important quantity to control when conducting a series of tests.
...
For a single-comparison FPR of $e$, the familywise FPR for $n$ comparisons is $1 - [1-e]^n$.
\Cref{fig:Accuracy} shows the familywise accuracy rate on the y-axis; if 90\% is an acceptable overall accuracy, 2 comparisons can be made when $e = 0.05$; this increases to 10 comparisons when $e =  0.01$.
Thus, at an error rate of \Sexpr{sprintf("e = %.2f", fpr)}, examiners can conduct approximately 5 comparisons without exceeding the familywise FPR of 10\%.

It is clear that under these constraints, the accuracy of an examination involving multiple comparisons between a wire and a tool will be low, as the number of candidate alignments that must be examined is high.
...
When we then consider that examiners would make cuts at multiple angles \citep{baikerToolmarkVariabilityQuality2015}, increasing the number of comparisons even in the simple case we have presented here, the probability of a false identification becomes even more likely.
...
In this short contribution, we have shown that the implicit comparisons performed during forensic analysis of wire cuts increase the familywise error rate.
...
First, examiners should report (or defense attorneys should request) the overall length or area of surfaces generated from blade cuts, test fires, and equivalent procedures intended to assess the marks made by a tool submitted for comparison with evidence.
Examiners should also report the total consecutive length or area of the recovered evidence. These pieces of information will allow for calculations of minimum and maximum number of comparisons (as demonstrated in this paper), facilitating calculation of overall error rates.
