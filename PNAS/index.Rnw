...
\hh{For a modestly sized database search of size 1000, the initial false positive rate should not exceed 1 in 10,000 to guarantee a family-wise total false identification error of at most 10\%. }
...
Even the most innocuous example (small blade, only 2 cutting surfaces, and a relatively large wire) involves a minimum of \Sexpr{2*ncomp_min} %completely independent
comparisons.
When we then consider that examiners would make cuts under multiple angles \citep{baikerToolmarkVariabilityQuality2015}, increasing the number of comparisons, the probability of a false identification becomes even more likely.
...
gt_tab <- data %>%
  ...
  gt() %>%
  tab_spanner(label = "False Positive Error (in %) based on N comparisons", columns = matches("E10")) %>%
  fmt_number(columns = c(E10, E100, E1000), decimals = 1) %>%
  fmt_number(columns = c(allowed), sep_mark = ",", decimals=0) %>%
  tab_header(
    title = md("False identification rates increase with the number of comparisons made.")
  ) %>%
 cols_label(
    E10 = "N = 10",
    E100 = "N = 100",
...
#   ylab("Family-wise false positive rate E") +
#   ggtitle("# Comparisons and Overall Accuracy") +
#   scale_color_discrete("FP Rate for \nsingle comparison") +
#   theme_bw() +
