...
\hh{Finding the best} \svp{alignment} \hh{requires implicitly to slide one source across the whole length (for one-dimensional sources, such as striations) or area (for two dimensional sources, such as impression marks) of the other source and keep track of the similarity measure.}
