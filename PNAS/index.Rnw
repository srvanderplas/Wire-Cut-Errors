...
%
\hh{In this paper, we discuss that very close non-matches are far from an unusual phenomenon  but a direct consequence of a large number of comparisons, and simply reflect the power of the search algorithms.}
%
\hh{In order to avoid embarassing false accusation and a further erosion of the public's trust in science, we need to address the problem of multiple comparisons by controlling their effect on false discovery rates.}
%
\hh{Database searches are an example where the number of searches is explicitly available as the number of objects meeting the search criteria. }
