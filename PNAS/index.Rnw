...
%\Cref{tab:Accuracy} shows the family-wise accuracy rate on the y-axis; if 90\% is an acceptable familywise accuracy, 2 comparisons can be made when $e = 0.05$; increasing to 10 comparisons when $e =  0.01$.
%Thus, at an error rate of \Sexpr{sprintf("e = %.2f", fpr)}, examiners can conduct comparisons before exceeding the family-wise FDR of 10\%.
...
\bottomrule
\end{tabular}}
\caption{\label{tab:family-wise}Table showing the relationship between false discovery rates and the chance of false identifications in $N$ comparisons for a set of different FDRs and different number of comparisons. The last column gives the number of comparisons allowed while ensuring a false identification percentage of at most 10\%.}
\end{table}
