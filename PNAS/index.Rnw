...
These concerns were raised in the past with regard to DNA \citep{thompson2003} and latent print evaluations  \citep{koehler2021}: one of the issues \citep{fine2006} leading to the wrongful accusation of Brandon Mayfield in the 2004 Madrid train bombing case was that the large size of the IAFIS database used to search for similar prints made it possible to locate `unusually' close non-matches.
The probability of finding  close non-matches for an item of evidence is directly linked to the size of the database available for searching: false "matches" occur more frequently as databases increase in size and algorithms are more powerful.
...
Compounding this issue, algorithmic results are typically based on a large number of comparisons that may not obvious to the user.
For example, a typical task in the forensic examination process is to find the best alignment between two items of evidence. For one-dimensional patterns, such as striation marks, this consists of sliding one item along the full length of the other item. For two-dimensional patterns, such as impression marks on an area, an additional rotation is involved. In practice, examiners visually rotate or align two surfaces under comparison microscopes. Algorithmically this visual search is mirrored by finding a maximum of the (area) cross-correlation function. The cross-correlation function was one of the first measures proposed \citep{vorburgerApplicationsCrosscorrelationFunctions2011} for quantifying the similarity  between two patterns in response to the  2009 NRC report \citep{nas2009}, and continues to be used in many pattern searching algorithms to find the best alignment between two images and  quantify their overall similarity.
...
In order to calculate the number of possible comparisons to be carried out in the course of the examination, we define $\ell$ to be the length of the blade cut, and $d$ to be the diameter of the wire. Here, we assume that striations  suitable for comparison are visible across the full width $d$ of the wire; if this is not the case, we adjust the value of $d$.
An illustration of the sliding comparison process is shown in \Cref{fig:wire-blade}. Imagine that a comparison starts from the left end of the blade and moves the wire surface $d$ units at a time along the blade.
At minimum, there must be $\ell/d$ comparisons in order to assess whether one side of the blade cut matches one side of the wire. We can generally assume that these comparisons are independent  because each comparison involves different surfaces. \hh{We need to further assume that the length $d$ is wide enough to cover some striations; this ensures that the width of $d$ is wider than any significant autocorrelation  observed for the striations on the blade}.
%no overlapping data is used.
\hh{Note, that this is a rather crude assessment of the bare minimal number of comparisons. }% Of course, this overlooks the possibility of partial matches and assumes perfect alignment between the diameter of the wire and the portion of the blade used to cut the wire, which is unrealistic.
...
At a maximum, both the blade and the wire are either digitally scanned at resolution of $r$ pixel per mm, or visually examined using a microscope with a digital resolution that can be expressed as equivalent to the digital scan.
...
Let us consider a wire-cutting tool with a 1.5 cm razor blade that meets a cast surface (one such tool is shown in \Cref{fig:tool-pic}); the wire is held against this rectangular cast surface as the blade is pushed into the wire, cutting it in two.
...
If the blade cuts and the wire edge are scanned at a lateral resolution of $\Sexpr{res*1000} \mu m$ per pixel, the resulting digital files will require \svp{approximately $\Sexpr{format(round(ncomp_max/1000)*1000, big.mark=',')}$ comparisons per blade cut; $\Sexpr{format(round(ncomp_max/1000)*2000, big.mark=',')}$ comparisons overall}.
