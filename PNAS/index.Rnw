...
However, in toolmark examination, no discipline-wide objective matching rule is used; examiners testify based on subjective, individual rules for how much similarity is enough for an identification.
As a result, it is not possible to separate coincidental match rates from human/technical failures.
<<fpr, include = F>>=
errors <- c(20, 56, 1)
knm <- c(2842, 774, 220)
...
It is clear that under these constraints, the accuracy of a forensic evaluation involving multiple comparisons between a wire and a tool is likely to be low, as the number of candidate alignments that must be examined is too high.
