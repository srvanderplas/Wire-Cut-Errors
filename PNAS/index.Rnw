...
Here, we consider the multiple comparisons problem that arises from a relatively simple toolmark comparison: matching a cut wire to a wire-cutting tool. We describe the comparison approach, estimate the (minimal) number of comparisons needed, and discuss how the false discovery rate changes with the number of comparisons involved, using error rates derived from published black-box studies.
...
#    parse = T, vjust = 1, hjust = 1) +
  geom_segment(data = res2sum,
     aes(x = N, xend=N, y = -Inf, yend=0.9), colour="grey30", linewidth=0.5) +
  geom_line(data = error_res, linetype = 2, linewidth = 0.75) +
  ylab("Probability false positive\n(N comparisons)") +
