...
If we begin by considering the multiple comparison problem relative to the minimal $\ell/d$ independent comparisons between a blade cut and a wire, we can compute the overall false positive rate for all comparisons using $e$, the false positive rate for one comparison, and simple probability calculations. Let $$F_n:=\{\text{at least one false pos. in }n\text{ disjoint comparisons}\}.$$ Then, $P(F_1) = e$, $P(\overline{F_1}) = 1-e$, $P(F_n) = [1-e]^n$, and $P(\overline{F_n}) = 1 - \left[P(\overline{F_1})\right]^n$.
